...
import { Settings as SettingsIcon, LogOut, Upload, Download, Plus, X, Save } from 'lucide-react'
...
export default function Settings({ onSignOut }) {
  ...
  const handleExport = async () => {
    try {
      const collections = ['income', 'expenses', 'myExpenses', 'myExpenseMonths', 'employees', 'payrollRuns', 'invoices', 'officeSetup', 'officeSetupTransactions', 'salaryLedger', 'inventory', 'carLoan', 'carLoanPayments', 'reminders']
      const backup = {}
      for (const col of collections) {
        backup[col] = await getDocuments(col)
      }
  ...
  const CategoryEditor = ({ label, field }) => (
    <div>
      <div className="flex items-center justify-between mb-2">
        <label className="text-xs text-[#888] font-body font-medium uppercase tracking-wider">{label}</label>
        <button onClick={() => addCat(field)} className="text-xs text-[#E8192C] hover:underline font-body flex items-center gap-1">
          <Plus size={10} /> Add
        </button>
      </div>
        ...
        {form[field].map((cat, i) => (
          <div key={i} className="flex items-center gap-2">
            <input
              ...
              className="flex-1 bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg px-3 py-1.5 text-sm text-white font-body outline-none focus:border-[#E8192C]"
            />
            <button onClick={() => removeCat(field, i)} className="w-7 h-7 rounded-lg hover:bg-red-500/10 flex items-center justify-center text-[#333] hover:text-red-400 transition-colors">
              <X size={12} />
            </button>
          </div>
        ))}
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-2xl">
      <SectionHeader
        ...
        action={
          <div className="flex gap-2">
            <Button variant="ghost" size="sm" onClick={handleExport} icon={Download}>Export Backup</Button>
            <Button size="sm" onClick={handleSave} loading={saving} icon={Save}>Save Changes</Button>
          </div>
        }
      />
      ...
      {/* Company profile */}
      <Card className="p-5">
        <h3 className="font-display font-bold text-white text-base mb-4">Company Profile</h3>
        <div className="space-y-4">
          {/* Logo */}
          <div>
            <label className="text-xs text-[#888] font-body font-medium uppercase tracking-wider block mb-2">Company Logo</label>
            <div className="flex items-center gap-4">
              {logoBase64 ? (
                <img src={logoBase64} alt="Logo" className="h-12 object-contain bg-[#1a1a1a] rounded-lg p-2" />
              ) : (
                <div className="h-12 w-24 bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg flex items-center justify-center">
                  <span className="text-[#333] text-xs font-body">No logo</span>
                </div>
              )}
              <label className="cursor-pointer">
                <input type="file" accept="image/*" onChange={handleLogoUpload} className="hidden" />
                <span className="text-xs text-[#555] hover:text-white border border-[#2a2a2a] hover:border-[#444] px-3 py-2 rounded-lg font-body transition-all flex items-center gap-1.5">
      ...
      {/* Opening balances */}
      <Card className="p-5">
        <h3 className="font-display font-bold text-white text-base mb-4">Opening Balances</h3>
      ...
      {/* Rent reminder */}
      <Card className="p-5">
        <h3 className="font-display font-bold text-white text-base mb-4">Rent Reminder</h3>
        <div className="grid grid-cols-2 gap-3">
      ...
      {/* Categories */}
      <Card className="p-5">
        <h3 className="font-display font-bold text-white text-base mb-5">Categories</h3>
      ...
      <Card className="p-5 border-red-500/10">
        <h3 className="font-display font-bold text-white text-base mb-4">Account</h3>
        <div className="space-y-3">
          <div className="flex items-center justify-between p-3 bg-[#1a1a1a] rounded-xl">
            <div>
              <div className="text-sm text-white font-body">Export Full Backup</div>
              <div className="text-xs text-[#444] font-body">Download all your data as JSON</div>
            </div>
          ...
          <div className="flex items-center justify-between p-3 bg-red-500/5 border border-red-500/10 rounded-xl">
            <div>
              <div className="text-sm text-white font-body">Sign Out</div>
              <div className="text-xs text-[#444] font-body">You'll need to sign in again to access the app</div>
            </div>
